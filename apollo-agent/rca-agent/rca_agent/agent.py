"""
rca_agent/agent.py
──────────────────
ReAct-style Root Cause Analysis agent built on Anthropic's tool-use API.

Supports both single-service and cross-service (upstream/downstream) RCA.

Flow:
  1. Load error log from the observability adapter
  2. Check dependency memory for known upstream services
  3. Resolve the GitHub repo via RepoResolver (CI/CD → service_repo_map → sub-agent)
  4. Run a ReAct loop (max MAX_REACT_ITERATIONS):
       - Call Claude with 13 tools registered
       - Dispatch tool calls to GitHub helpers / cache / memory / deployment adapter
       - Break on stop_reason == "end_turn" OR finish_rca tool call
  5. After _FinishRCA: flush staged dependency memory write
  6. Validate & persist the RCA report, then return it
"""

import json
import logging
import uuid
from datetime import datetime, timezone
from typing import Callable

import anthropic

from .cache import read_cache, write_cache, append_rca_history
from .config import settings
from .db import execute, log_token_usage
from .dependency_memory import read_dependency_memory, write_dependency_memory
from .token_pricing import compute_cost
from .github_tools import (
    get_commit_diff,
    get_commits_since,
    get_repo_file,
    list_repo_files,
    search_code_in_repo,
    search_github_global,
)
from .models import RCAReport
from .report import validate_report
from .repo_resolver import RepoResolver

logger = logging.getLogger(__name__)

# Patterns that identify chaos / fault-injection test files.
# These intentionally throw exceptions to simulate failures — they are never root causes.
_CHAOS_PATTERNS = (
    "chaoscontroller",
    "chaosexception",
    "/chaos/",
    "chaos_controller",
    "faultinject",
    "fault_inject",
)

def _is_chaos_path(path: str) -> bool:
    pl = path.lower()
    return any(p in pl for p in _CHAOS_PATTERNS)


# ── Internal sentinel exception ───────────────────────────────────────────────

class _FinishRCA(Exception):
    """Raised by _dispatch when Claude calls finish_rca to exit the ReAct loop."""
    def __init__(self, report: RCAReport):
        self.report = report


# ── System prompt ─────────────────────────────────────────────────────────────

SYSTEM_PROMPT = """You are an expert Site Reliability Engineer performing automated Root Cause Analysis (RCA).

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HARD CONSTRAINT — CHAOS / FAULT-INJECTION (read before doing anything else)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ChaosController, ChaosException, and any file under a /chaos/ or fault_inject path are
intentional test-scaffolding that deliberately throw exceptions to simulate failures.
They are ALWAYS the trigger — NEVER the root cause.

ABSOLUTE RULES (no exceptions, enforced by the tool layer):
1. Chaos frames in a stack trace are NOISE. Discard every ChaosController / ChaosException
   frame immediately and investigate the real business-logic class beneath it
   (e.g., AccountService, OrderService, PricingService).
2. NEVER request a chaos file via get_repo_file. The tool will reject it and return an advisory.
   When you receive that advisory, obey it — move on to the actual business code.
3. Chaos paths returned by search results must be ignored. Do not click through, do not cite them.
4. root_cause.code_reference.file in your final report MUST point to real business or
   infrastructure code. A chaos path in that field is an invalid report.
5. If the entire stack trace consists of chaos frames, identify the service under test and
   investigate its own source code and dependencies for the real cause.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

CORE PRINCIPLE: An error in service A is sometimes caused by a breaking change in service B (upstream).
Always consider cross-service causation before concluding the root cause is local.

STARTUP: Before investigating code, call read_dependency_memory for the affected service.
If a known upstream dependency is returned, start your investigation there.

PROCESS:
1. Call read_dependency_memory — check if a known upstream dependency caused this.
2. Read the error log carefully — note error type, stack trace frames (file + line), service name, environment, timestamp.
3. Call get_recent_deployments to find what changed recently in the affected service/environment.
4. Fetch the exact files named in the stack trace using get_repo_file (always check read_context_cache first).
5. If the error is a null/missing field, search for where that field is defined: use search_code_in_repo on the calling service, then search_github_global to find the upstream service that provides it.
6. If a cross-service cause is found: fetch the upstream file, call get_commit_diff on the upstream rename/change commit, write_dependency_memory BEFORE calling finish_rca.
7. When you have sufficient evidence, call finish_rca with a complete, structured report.

RULES:
- Always call read_context_cache before any GitHub API tool. Always call write_context_cache after a successful GitHub fetch.
- Never speculate beyond what you can directly observe in code, diffs, or deployment records.
- Never suggest executing code or making live changes — only provide recommendations.
- finish_rca is the ONLY way to end the analysis loop — you MUST call it.
- write_dependency_memory MUST be called BEFORE finish_rca (never after — the loop exits on finish_rca).
- Confidence levels: "high" = direct evidence in diff/code; "medium" = strong inference from context; "low" = circumstantial.
- Keep suggested_solutions practical, prioritised (1 = most urgent), and actionable.
- CHAOS FILES: See hard constraint at the top of this prompt. Chaos frames are always triggers, never root causes. The tool layer blocks chaos file fetches and filters chaos paths from search results — obey those signals immediately.
- LINE NUMBERS: File content is returned with explicit line numbers in the format "  N | <source line>". Always read these prefixed numbers — do not count lines yourself. When reporting code_reference.line, use the N from the prefix of the exact line you are citing.

RCA REPORT SCHEMA (pass as JSON to finish_rca):
{
  "schema_version": "1.1",
  "rca_id": "<auto-generated uuid4>",
  "error_log_id": "<from the input>",
  "service_name": "<service name>",
  "generated_at": "<ISO 8601 UTC>",
  "service_boundary_crossed": false,
  "upstream_service": null,
  "incident_summary": {
    "what": "<one sentence>",
    "when": "<ISO timestamp>",
    "environment": "<prod/staging/etc>",
    "severity": "<critical/high/medium/low>",
    "estimated_impact": "<optional>"
  },
  "timeline": [{"timestamp": "<ISO>", "event": "<description>"}],
  "root_cause": {
    "summary": "<clear explanation of what caused the error>",
    "confidence": "high|medium|low",
    "confidence_reason": "<why this confidence level>",
    "code_reference": {
      "file": "<path>",
      "line": <int or null>,
      "function": "<name or null>",
      "repo": "<repo name>",
      "commit_sha": "<sha or null>",
      "github_url": "<url or null>"
    },
    "regression_introduced_by": {
      "commit_sha": "<sha>",
      "commit_message": "<msg>",
      "author": "<name>",
      "deployed_at": "<ISO>"
    } or null
  },
  "contributing_factors": ["<factor>"],
  "evidence": [{"type": "code|diff|deployment|log", "description": "<desc>", "value": "<snippet>"}],
  "impact_assessment": {
    "affected_service": "<name>",
    "affected_environment": "<env>",
    "affected_functionality": "<optional>",
    "inferred_error_rate": "<optional>",
    "duration_estimate": "<optional>"
  },
  "suggested_solutions": [
    {"priority": 1, "effort": "quick-fix|medium|large", "title": "<title>", "description": "<desc>", "code_suggestion": "<optional code snippet>"}
  ],
  "prevention_recommendations": ["<recommendation>"],
  "analysis_metadata": {
    "model": "<model>",
    "react_iterations": <int>,
    "github_files_fetched": ["<path>"],
    "cache_hits": ["<key>"],
    "repos_investigated": ["<org/repo>"],
    "deployment_record_used": <bool>,
    "repo_discovered_via": "service_repo_map|cicd|sub_agent|cicd+mapping"
  }
}"""

# ── Tool definitions ──────────────────────────────────────────────────────────

TOOLS = [
    {
        "name": "get_repo_file",
        "description": (
            "Fetch the full content of a file from a GitHub repository at a given ref "
            "(branch name or commit SHA). Always call read_context_cache first."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "org": {"type": "string", "description": "GitHub organisation name"},
                "repo": {"type": "string", "description": "Repository name"},
                "path": {"type": "string", "description": "File path within the repo"},
                "ref": {
                    "type": "string",
                    "description": "Branch name or commit SHA (default: main)",
                    "default": "main",
                },
            },
            "required": ["org", "repo", "path"],
        },
    },
    {
        "name": "list_repo_files",
        "description": (
            "List files and directories at a path in a GitHub repo. Useful for exploring "
            "the repo structure before fetching specific files."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "org": {"type": "string"},
                "repo": {"type": "string"},
                "directory": {
                    "type": "string",
                    "description": "Directory path to list (empty string = root)",
                    "default": "",
                },
                "ref": {"type": "string", "default": "main"},
            },
            "required": ["org", "repo"],
        },
    },
    {
        "name": "search_code_in_repo",
        "description": (
            "Search for a string, symbol, or class name within a single GitHub repository. "
            "Returns matching file paths and URLs. "
            "Use search_github_global to search across all repos in an org."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "org": {"type": "string"},
                "repo": {"type": "string"},
                "query": {"type": "string", "description": "Search term or symbol"},
            },
            "required": ["org", "repo", "query"],
        },
    },
    {
        "name": "get_commit_diff",
        "description": (
            "Get the full diff (changed files, patches) for a specific commit SHA. "
            "Critical for identifying regressions introduced by recent deployments."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "org": {"type": "string"},
                "repo": {"type": "string"},
                "commit_sha": {"type": "string", "description": "Full or abbreviated commit SHA"},
            },
            "required": ["org", "repo", "commit_sha"],
        },
    },
    {
        "name": "get_commits_since",
        "description": (
            "List commits on a branch since a given timestamp. Useful for finding all "
            "changes that could have contributed to the error."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "org": {"type": "string"},
                "repo": {"type": "string"},
                "branch": {"type": "string"},
                "since": {
                    "type": "string",
                    "description": "ISO 8601 timestamp (e.g. 2026-05-13T00:00:00Z)",
                },
            },
            "required": ["org", "repo", "branch", "since"],
        },
    },
    {
        "name": "get_recent_deployments",
        "description": (
            "Retrieve recent successful deployments for a service/environment pair. "
            "Returns deployment records including commit SHA, branch, deployer, and pipeline URL."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "service_name": {"type": "string"},
                "environment": {"type": "string"},
                "since_hours": {
                    "type": "integer",
                    "description": "How many hours back to look (default: 48)",
                    "default": 48,
                },
                "limit": {"type": "integer", "default": 5},
            },
            "required": ["service_name", "environment"],
        },
    },
    {
        "name": "get_service_metadata",
        "description": "Get metadata about a service from the observability platform.",
        "input_schema": {
            "type": "object",
            "properties": {
                "service_name": {"type": "string"},
            },
            "required": ["service_name"],
        },
    },
    {
        "name": "read_context_cache",
        "description": (
            "Read a previously cached GitHub file or repo tree from the database. "
            "ALWAYS call this before get_repo_file or list_repo_files to avoid redundant API calls."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "service_name": {"type": "string"},
                "cache_key": {
                    "type": "string",
                    "description": "Use 'file:<path>' for files, 'repo_tree' for directory listings",
                },
            },
            "required": ["service_name", "cache_key"],
        },
    },
    {
        "name": "write_context_cache",
        "description": (
            "Persist a GitHub file or repo tree to the cache. "
            "ALWAYS call this after a successful get_repo_file or list_repo_files call."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "service_name": {"type": "string"},
                "cache_key": {"type": "string"},
                "content": {
                    "type": "object",
                    "description": "The content object to cache (file content, tree listing, etc.)",
                },
                "commit_sha": {
                    "type": "string",
                    "description": "Optional commit SHA to associate with the cache entry",
                },
            },
            "required": ["service_name", "cache_key", "content"],
        },
    },
    {
        "name": "search_github_global",
        "description": (
            "Search code across ALL repositories in a GitHub org. "
            "Use this to find which upstream service defines a field, class, or endpoint "
            "that the failing service depends on. "
            "Example: search for 'discountRate' to find the service that renamed the field."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Code search term (e.g. 'discountRate PricingDto')",
                },
                "org": {
                    "type": "string",
                    "description": "GitHub organisation to scope the search (strongly recommended)",
                },
                "max_results": {
                    "type": "integer",
                    "description": "Maximum results to return (default: 15)",
                    "default": 15,
                },
            },
            "required": ["query"],
        },
    },
    {
        "name": "read_dependency_memory",
        "description": (
            "Read known upstream service dependencies from persistent memory. "
            "ALWAYS call this at the START of every RCA before fetching any code. "
            "If a known upstream is returned, investigate it first."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "downstream_service": {
                    "type": "string",
                    "description": "Service name to look up (the one with the error). "
                                   "Omit to read all known dependencies.",
                },
            },
            "required": [],
        },
    },
    {
        "name": "write_dependency_memory",
        "description": (
            "Persist a newly discovered service dependency to memory. "
            "Call this when you discover that the error in service A was caused by a "
            "breaking change in service B. "
            "IMPORTANT: call this BEFORE finish_rca — it cannot be called after."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "downstream_service": {
                    "type": "string",
                    "description": "The service with the bug (the one that calls upstream)",
                },
                "upstream_service": {
                    "type": "string",
                    "description": "The upstream service whose change caused the issue",
                },
                "upstream_repo": {
                    "type": "string",
                    "description": "GitHub repo name for the upstream service",
                },
                "evidence": {
                    "type": "string",
                    "description": "Short description of how the dependency was discovered",
                },
                "breaking_change": {
                    "type": "string",
                    "description": "Optional: describe the breaking change found (e.g. field rename)",
                },
                "upstream_branch": {
                    "type": "string",
                    "description": "Branch to use when fetching upstream source (default: main)",
                    "default": "main",
                },
            },
            "required": ["downstream_service", "upstream_service", "upstream_repo", "evidence"],
        },
    },
    {
        "name": "finish_rca",
        "description": (
            "Submit the completed RCA report. This MUST be called to end the analysis. "
            "Pass the full report JSON conforming to the RCAReport schema v1.1. "
            "If a cross-service root cause was found, set service_boundary_crossed=true "
            "and upstream_service to the upstream service name. "
            "IMPORTANT: call write_dependency_memory BEFORE this if a new cross-service "
            "dependency was discovered."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "report": {
                    "type": "object",
                    "description": "Full RCA report object conforming to RCAReport schema v1.1",
                }
            },
            "required": ["report"],
        },
    },
]


# ── Agent class ───────────────────────────────────────────────────────────────

class RCAAgent:
    def __init__(self, obs_adapter, cicd_adapter):
        self.obs = obs_adapter
        self.cicd = cicd_adapter
        # Base client uses env key; per-run client is created in run() with effective key
        self._base_api_key = settings.anthropic_api_key

    def _emit(self, callback: Callable[[dict], None] | None, event: dict) -> None:
        """Emit a trace event if a callback is registered."""
        if callback is None:
            return
        try:
            event.setdefault("ts", datetime.now(timezone.utc).isoformat())
            callback(event)
        except Exception:
            pass  # trace errors must never crash the agent

    def run(
        self,
        error_log_id: str,
        trace_callback: Callable[[dict], None] | None = None,
        # Runtime overrides — callers pass effective values (env + Settings UI overlay)
        api_key: str | None = None,
        model: str | None = None,
        max_iterations: int | None = None,
    ) -> RCAReport:
        # Resolve effective settings: caller overrides take priority over env defaults
        effective_key   = api_key        or settings.anthropic_api_key
        effective_model = model          or settings.model
        effective_iters = max_iterations or settings.max_react_iterations

        # Fresh client per run — picks up any runtime key change immediately
        client = anthropic.Anthropic(api_key=effective_key)

        # 1. Load error log
        error_log = self.obs.get_error_log(error_log_id)

        # Emit start event
        self._emit(trace_callback, {
            "type": "start",
            "service": error_log.service_name,
            "error_type": error_log.error_type,
        })

        # 2. Resolve repo
        resolver = RepoResolver(self.cicd)

        # Check if sub-agent will be needed (service has no mapping)
        discovered_via_sub_agent = False
        try:
            from .db import execute_one as _eo
            _mapping = _eo(
                "SELECT service_name FROM service_repo_map WHERE service_name=%s",
                (error_log.service_name,),
            )
            if _mapping is None:
                discovered_via_sub_agent = True
        except Exception:
            pass

        if discovered_via_sub_agent:
            self._emit(trace_callback, {
                "type": "sub_agent",
                "action": f"launching repo discovery for {error_log.service_name}",
            })

        repo_info = resolver.resolve(
            error_log.service_name, error_log.environment, error_log.occurred_at
        )
        logger.info(
            "Resolved repo for %s: %s/%s (via %s)",
            error_log.service_name,
            repo_info["org"],
            repo_info["repo"],
            repo_info["discovered_via"],
        )

        # 3. Build initial context message
        error_context = {
            "error_log_id": error_log_id,
            "service_name": error_log.service_name,
            "environment": error_log.environment,
            "error_type": error_log.error_type,
            "error_message": error_log.error_message,
            "stack_trace": error_log.stack_trace,
            "severity": error_log.severity,
            "occurred_at": error_log.occurred_at.isoformat(),
            "metadata": error_log.metadata,
            "resolved_repo": repo_info,
        }

        messages = [
            {
                "role": "user",
                "content": (
                    "Perform a Root Cause Analysis for the following error. "
                    "Start by calling read_dependency_memory for this service, then use "
                    "the available tools to investigate, then call finish_rca.\n\n"
                    f"```json\n{json.dumps(error_context, indent=2, default=str)}\n```"
                ),
            }
        ]

        # 4. ReAct loop state
        iterations = 0
        github_files_fetched: list[str] = []
        cache_hits: list[str] = []
        repos_investigated: list[str] = []
        _pending_memory_write: dict | None = None   # staged by write_dependency_memory tool
        final_report: RCAReport | None = None

        while iterations < effective_iters:
            iterations += 1
            logger.debug("ReAct iteration %d", iterations)

            try:
                response = client.messages.create(
                    model=effective_model,
                    max_tokens=4096,
                    system=SYSTEM_PROMPT,
                    tools=TOOLS,
                    messages=messages,
                )
            except anthropic.AuthenticationError:
                raise RuntimeError(
                    "Invalid Anthropic API key. Update it in Settings → Claude AI."
                )
            except anthropic.PermissionDeniedError:
                raise RuntimeError(
                    "Anthropic API key lacks permission. Check your key at console.anthropic.com."
                )
            except anthropic.RateLimitError:
                raise RuntimeError(
                    "Anthropic rate limit reached. Increase your usage limit at "
                    "console.anthropic.com → Billing → Limits."
                )
            except anthropic.BadRequestError as _bre:
                _msg = str(_bre)
                if "usage limits" in _msg.lower() or "regain access" in _msg.lower():
                    raise RuntimeError(
                        f"Anthropic monthly usage limit reached. {_msg} "
                        "Increase it at console.anthropic.com → Billing → Limits."
                    )
                raise

            # Record real token counts from the API response
            try:
                u = response.usage
                cache_r = getattr(u, "cache_read_input_tokens", 0) or 0
                cache_w = getattr(u, "cache_creation_input_tokens", 0) or 0
                log_token_usage(
                    model=effective_model,
                    source="rca_agent",
                    input_tokens=u.input_tokens,
                    output_tokens=u.output_tokens,
                    error_log_id=error_log_id,
                    cache_read_tokens=cache_r,
                    cache_creation_tokens=cache_w,
                    estimated_cost_usd=compute_cost(
                        effective_model, u.input_tokens, u.output_tokens, cache_r, cache_w
                    ),
                    iteration_num=iterations,
                )
            except Exception:
                pass  # never crash the agent on logging failures

            # Append assistant turn
            messages.append({"role": "assistant", "content": response.content})

            # Emit reasoning text blocks (text between tool calls)
            for block in response.content:
                if hasattr(block, "type") and block.type == "text" and block.text.strip():
                    self._emit(trace_callback, {
                        "type": "reasoning",
                        "text": block.text.strip(),
                    })

            if response.stop_reason == "end_turn":
                logger.warning(
                    "Agent reached end_turn at iteration %d without calling finish_rca — "
                    "will force a summary report", iterations
                )
                break  # falls through to force-report block below

            if response.stop_reason != "tool_use":
                logger.warning(
                    "Unexpected stop_reason=%s at iteration %d — will force a summary report",
                    response.stop_reason, iterations
                )
                break  # falls through to force-report block below

            # Dispatch tool calls
            tool_results = []
            done = False

            for block in response.content:
                if block.type != "tool_use":
                    continue

                tool_name = block.name
                tool_input = block.input
                result: dict

                # Emit tool_call event (before execution)
                self._emit(trace_callback, {
                    "type": "tool_call",
                    "tool": tool_name,
                    "input": tool_input,
                })

                try:
                    result = self._dispatch(
                        tool_name,
                        tool_input,
                        error_log,
                        repo_info,
                        github_files_fetched,
                        cache_hits,
                        repos_investigated,
                        pending_memory_write_ref=[_pending_memory_write],
                        trace_callback=trace_callback,
                        effective_model=effective_model,
                    )
                    # Update staged memory write ref (mutable via list wrapper)
                    # _dispatch signals a staged write by returning a sentinel key
                    if tool_name == "write_dependency_memory" and "status" in result:
                        _pending_memory_write = tool_input  # stage it
                except _FinishRCA as fin:
                    final_report = fin.report
                    done = True
                    tool_results.append(
                        {
                            "type": "tool_result",
                            "tool_use_id": block.id,
                            "content": json.dumps({"status": "rca_submitted"}),
                        }
                    )
                    break
                except Exception as exc:
                    logger.exception("Tool %s raised: %s", tool_name, exc)
                    result = {"error": str(exc)}

                # Emit tool_result event (after execution)
                self._emit(trace_callback, {
                    "type": "tool_result",
                    "tool": tool_name,
                    "summary": self._summarize_result(tool_name, tool_input, result),
                })

                tool_results.append(
                    {
                        "type": "tool_result",
                        "tool_use_id": block.id,
                        "content": json.dumps(result, default=str),
                    }
                )

            messages.append({"role": "user", "content": tool_results})

            if done:
                break

        if final_report is None:
            logger.warning(
                "Max iterations (%d) reached without finish_rca — forcing quick report", iterations
            )
            self._emit(trace_callback, {
                "type": "reasoning",
                "text": (
                    f"Reached the maximum of {effective_iters} analysis iterations. "
                    "Generating a summary report from the evidence gathered so far..."
                ),
            })
            try:
                messages.append({
                    "role": "user",
                    "content": (
                        f"You have used all {effective_iters} allowed iterations. "
                        "You MUST call finish_rca RIGHT NOW with your best analysis based on "
                        "everything you have gathered so far. "
                        "If root cause is uncertain, set confidence to 'low' and clearly state "
                        "what evidence is still missing. "
                        "Do NOT call any other tool — only finish_rca."
                    ),
                })
                force_resp = client.messages.create(
                    model=effective_model,
                    max_tokens=4096,
                    system=SYSTEM_PROMPT,
                    tools=TOOLS,
                    messages=messages,
                )
                for blk in force_resp.content:
                    if (
                        hasattr(blk, "type")
                        and blk.type == "tool_use"
                        and blk.name == "finish_rca"
                    ):
                        try:
                            self._dispatch(
                                blk.name,
                                blk.input,
                                error_log,
                                repo_info,
                                github_files_fetched,
                                cache_hits,
                                repos_investigated,
                                pending_memory_write_ref=[_pending_memory_write],
                                trace_callback=trace_callback,
                                effective_model=effective_model,
                            )
                        except _FinishRCA as fin:
                            final_report = fin.report
                        break
            except Exception as exc:
                logger.error("Force-report API call failed: %s", exc, exc_info=True)

        if final_report is None:
            raise RuntimeError(
                f"RCA agent completed {iterations} iteration(s) without calling finish_rca, "
                "and the forced summary report also failed. "
                "Check logs for details — likely a GitHub PAT or API key issue."
            )

        # 5. Flush staged dependency memory write (must happen AFTER loop exits)
        if _pending_memory_write:
            try:
                write_dependency_memory(
                    downstream_service=_pending_memory_write["downstream_service"],
                    upstream_service=_pending_memory_write["upstream_service"],
                    upstream_repo=_pending_memory_write["upstream_repo"],
                    evidence=_pending_memory_write["evidence"],
                    breaking_change=_pending_memory_write.get("breaking_change"),
                    upstream_branch=_pending_memory_write.get("upstream_branch", "main"),
                )
                logger.info(
                    "Flushed dependency memory: %s → %s",
                    _pending_memory_write["downstream_service"],
                    _pending_memory_write["upstream_service"],
                )
            except Exception as exc:
                logger.warning("Could not flush dependency memory: %s", exc)

        # 6. Update metadata
        final_report.analysis_metadata.react_iterations = iterations
        final_report.analysis_metadata.github_files_fetched = github_files_fetched
        final_report.analysis_metadata.cache_hits = cache_hits
        final_report.analysis_metadata.repos_investigated = repos_investigated
        final_report.analysis_metadata.deployment_record_used = repo_info.get(
            "deployment_record_used", False
        )
        final_report.analysis_metadata.repo_discovered_via = repo_info.get(
            "discovered_via", "unknown"
        )

        # Emit complete event
        self._emit(trace_callback, {
            "type": "complete",
            "confidence": final_report.root_cause.confidence,
            "iterations": iterations,
            "cross_service": final_report.service_boundary_crossed,
        })

        # Persist (best-effort — app/main.py also persists)
        self._persist(final_report, error_log_id)
        return final_report

    # ── Tool dispatcher ───────────────────────────────────────────────────────

    def _summarize_result(self, tool_name: str, inp: dict, result: dict) -> str:
        """Generate a short human-readable summary of a tool result."""
        if "error" in result:
            return f"Error from {tool_name}: {result['error']}"
        if tool_name == "get_repo_file":
            path = inp.get("path", "")
            content = result.get("content", "")
            lines = len(content.split("\n")) if isinstance(content, str) else "?"
            return f"Fetched {lines} lines from {path.split('/')[-1]}"
        if tool_name == "list_repo_files":
            items = result.get("files", result.get("tree", []))
            count = len(items) if isinstance(items, list) else "?"
            return f"Listed {count} items in {inp.get('directory', '/') or '/'}"
        if tool_name == "search_code_in_repo":
            items = result.get("results", result.get("items", []))
            count = len(items) if isinstance(items, list) else "?"
            return f"Found {count} results for '{inp.get('query', '')}'"
        if tool_name == "search_github_global":
            count = result.get("count", len(result.get("results", [])))
            return f"Global search '{inp.get('query', '')}' → {count} results"
        if tool_name == "get_commit_diff":
            sha = inp.get("commit_sha", "")[:8]
            files = result.get("files", [])
            additions = sum(f.get("additions", 0) for f in files) if isinstance(files, list) else 0
            deletions = sum(f.get("deletions", 0) for f in files) if isinstance(files, list) else 0
            return f"Commit {sha} diff: {len(files)} files changed (+{additions}/-{deletions})"
        if tool_name == "get_commits_since":
            commits = result.get("commits", [])
            count = len(commits) if isinstance(commits, list) else "?"
            return f"Found {count} commits since {inp.get('since', '')[:10]}"
        if tool_name == "get_recent_deployments":
            deps = result.get("deployments", [])
            return f"Found {len(deps)} recent deployment(s) for {inp.get('service_name', '')}"
        if tool_name == "get_service_metadata":
            return f"Fetched metadata for {inp.get('service_name', '')}"
        if tool_name == "read_context_cache":
            if result.get("cache_hit"):
                return f"Cache hit: {inp.get('cache_key', '')}"
            return f"Cache miss: {inp.get('cache_key', '')}"
        if tool_name == "write_context_cache":
            return f"Cached {inp.get('cache_key', '')} for {inp.get('service_name', '')}"
        if tool_name == "read_dependency_memory":
            if result.get("found"):
                deps = result.get("dependencies", {})
                upstreams = [v.get("upstream", "?") for v in deps.values() if isinstance(v, dict)]
                return f"Known dependency memory: {', '.join(upstreams) or 'found'}"
            return "No known upstream dependencies in memory"
        if tool_name == "write_dependency_memory":
            return (
                f"Staged dependency: {inp.get('downstream_service', '?')} → "
                f"{inp.get('upstream_service', '?')}"
            )
        return f"{tool_name} completed"

    def _dispatch(
        self,
        name: str,
        inp: dict,
        error_log,
        repo_info: dict,
        github_files_fetched: list[str],
        cache_hits: list[str],
        repos_investigated: list[str],
        pending_memory_write_ref: list,
        trace_callback: Callable[[dict], None] | None = None,
        effective_model: str = "",
    ) -> dict:

        if name == "get_repo_file":
            path = inp["path"]
            org = inp.get("org", repo_info["org"])
            repo = inp.get("repo", repo_info["repo"])

            # Chaos/fault-injection files are test scaffolding — never root causes.
            # Return an advisory instead of the file so the agent cannot cite them.
            if _is_chaos_path(path):
                return {
                    "path": path,
                    "advisory": "chaos_file",
                    "content": (
                        f"⚠️  CHAOS / FAULT-INJECTION FILE — NOT the root cause.\n"
                        f"'{path}' is intentional fault-injection code that deliberately "
                        "throws exceptions to simulate production failures.\n"
                        "STOP — do not analyse this file. Investigate the real business-logic "
                        "class it was exercising when the exception was thrown "
                        "(e.g., AccountService, OrderService, PricingService)."
                    ),
                }

            result = get_repo_file(
                org, repo, path,
                inp.get("ref", repo_info.get("commit_sha") or "main"),
            )
            if "error" not in result:
                github_files_fetched.append(path)
                repo_key = f"{org}/{repo}"
                if repo_key not in repos_investigated:
                    repos_investigated.append(repo_key)

                # Add explicit line numbers so the agent reads the N from the prefix
                # instead of counting lines itself (which leads to off-by-N errors).
                # Truncate by line count, not by character, to keep line numbers intact.
                raw_lines = result["content"].split("\n")
                total_lines = len(raw_lines)
                MAX_LINES = 150
                if total_lines > MAX_LINES:
                    numbered = "\n".join(
                        f"{i + 1:5d} | {line}" for i, line in enumerate(raw_lines[:MAX_LINES])
                    )
                    numbered += (
                        f"\n      | ... [{total_lines - MAX_LINES} more lines not shown"
                        " — use search_code_in_repo to find specific symbols]"
                    )
                else:
                    numbered = "\n".join(
                        f"{i + 1:5d} | {line}" for i, line in enumerate(raw_lines)
                    )
                result = dict(result)
                result["content"] = numbered
                result["total_lines"] = total_lines
            return result

        if name == "list_repo_files":
            org = inp.get("org", repo_info["org"])
            repo = inp.get("repo", repo_info["repo"])
            result = list_repo_files(
                org, repo,
                inp.get("directory", ""),
                inp.get("ref", repo_info.get("commit_sha") or "main"),
            )
            if "error" not in result:
                repo_key = f"{org}/{repo}"
                if repo_key not in repos_investigated:
                    repos_investigated.append(repo_key)
                # Remove chaos/fault-injection files from the listing so the agent
                # never fetches them during directory exploration.
                files = result.get("files", [])
                filtered = [f for f in files if not _is_chaos_path(f.get("path", ""))]
                if len(filtered) < len(files):
                    result = dict(result)
                    result["files"] = filtered
                    result["chaos_files_hidden"] = len(files) - len(filtered)
            return result

        if name == "search_code_in_repo":
            result = search_code_in_repo(
                inp.get("org", repo_info["org"]),
                inp.get("repo", repo_info["repo"]),
                inp["query"],
            )
            if "results" in result:
                items = result["results"]
                filtered = [r for r in items if not _is_chaos_path(r.get("path", ""))]
                if len(filtered) < len(items):
                    result = dict(result)
                    result["results"] = filtered
                    result["chaos_files_hidden"] = len(items) - len(filtered)
            return result

        if name == "get_commit_diff":
            org = inp.get("org", repo_info["org"])
            repo = inp.get("repo", repo_info["repo"])
            result = get_commit_diff(org, repo, inp["commit_sha"])
            if "error" not in result:
                repo_key = f"{org}/{repo}"
                if repo_key not in repos_investigated:
                    repos_investigated.append(repo_key)
                # Truncate oversized patches to keep context window manageable
                for f in result.get("files", []):
                    patch = f.get("patch", "")
                    if len(patch) > 2000:
                        f["patch"] = patch[:2000] + f"\n... [truncated, {len(patch)} chars total]"
            return result

        if name == "get_commits_since":
            return get_commits_since(
                inp.get("org", repo_info["org"]),
                inp.get("repo", repo_info["repo"]),
                inp["branch"],
                inp["since"],
            )

        if name == "search_github_global":
            result = search_github_global(
                query=inp["query"],
                org=inp.get("org", repo_info.get("org")),
                max_results=inp.get("max_results", 15),
            )
            if "results" in result:
                items = result["results"]
                filtered = [r for r in items if not _is_chaos_path(r.get("path", ""))]
                if len(filtered) < len(items):
                    result = dict(result)
                    result["results"] = filtered
                    result["count"] = len(filtered)
                    result["chaos_files_hidden"] = len(items) - len(filtered)
            return result

        if name == "get_recent_deployments":
            from datetime import timedelta
            since_hours = inp.get("since_hours", 48)
            since = datetime.now(timezone.utc) - timedelta(hours=since_hours)
            records = self.cicd.get_recent_deployments(
                inp["service_name"],
                inp["environment"],
                since,
                inp.get("limit", 5),
            )
            return {
                "deployments": [
                    {
                        "service_name": r.service_name,
                        "environment": r.environment,
                        "branch": r.branch,
                        "commit_sha": r.commit_sha,
                        "commit_message": r.commit_message,
                        "deployed_at": r.deployed_at.isoformat(),
                        "deployer": r.deployer,
                        "pipeline_id": r.pipeline_id,
                        "pipeline_url": r.pipeline_url,
                        "github_repo": r.github_repo,
                        "status": r.status,
                    }
                    for r in records
                ]
            }

        if name == "get_service_metadata":
            return self.obs.get_service_metadata(inp["service_name"])

        if name == "read_context_cache":
            cached = read_cache(inp["service_name"], inp["cache_key"])
            if cached is not None:
                cache_hits.append(inp["cache_key"])
                self._emit(trace_callback, {"type": "cache_hit", "key": inp["cache_key"]})
                return {"cache_hit": True, "content": cached}
            return {"cache_hit": False}

        if name == "write_context_cache":
            write_cache(
                inp["service_name"],
                inp["cache_key"],
                inp["content"],
                inp.get("commit_sha"),
            )
            return {"status": "cached"}

        if name == "read_dependency_memory":
            return read_dependency_memory(inp.get("downstream_service"))

        if name == "write_dependency_memory":
            # Stage the write — actual disk write happens in run() after _FinishRCA
            # so the memory file is written even though finish_rca exits the loop.
            # Return a preview so Claude can confirm it was accepted.
            return {
                "status": "staged",
                "downstream": inp.get("downstream_service"),
                "upstream": inp.get("upstream_service"),
                "note": "Dependency will be persisted after finish_rca is called.",
            }

        if name == "finish_rca":
            # Claude passes {"report": {<full report fields>}}
            report_data = inp.get("report", inp)

            # ── Guaranteed scalar fields ─────────────────────────────────────
            report_data.setdefault("schema_version", "1.1")
            report_data.setdefault("rca_id", str(uuid.uuid4()))
            report_data.setdefault("error_log_id", error_log.id)
            report_data.setdefault("service_name", error_log.service_name)
            report_data.setdefault("generated_at", datetime.now(timezone.utc).isoformat())
            report_data.setdefault("service_boundary_crossed", False)
            report_data.setdefault("upstream_service", None)
            report_data.setdefault("contributing_factors", [])
            report_data.setdefault("evidence", [])
            report_data.setdefault("prevention_recommendations", [])

            # ── analysis_metadata — MUST be set up first; used by root_cause fallback below ──
            report_data.setdefault("analysis_metadata", {})
            meta = report_data["analysis_metadata"]
            meta["github_files_fetched"] = github_files_fetched
            meta["cache_hits"]           = cache_hits
            meta["repos_investigated"]   = repos_investigated
            meta["model"]                = effective_model
            meta.setdefault("react_iterations", 0)
            meta.setdefault("deployment_record_used", False)
            meta.setdefault("repo_discovered_via", "unknown")

            # ── Required nested objects — safe fallbacks so Pydantic never fails ──
            occurred = error_log.occurred_at.isoformat() if hasattr(error_log.occurred_at, "isoformat") else str(error_log.occurred_at)
            report_data.setdefault("incident_summary", {
                "what": error_log.error_message or error_log.error_type or "Unknown error",
                "when": occurred,
                "environment": error_log.environment or "production",
                "severity": error_log.severity or "ERROR",
            })
            report_data.setdefault("timeline", [
                {"timestamp": occurred, "event": f"{error_log.error_type} occurred in {error_log.service_name}"}
            ])
            if "root_cause" not in report_data:
                report_data["root_cause"] = {
                    "summary": (
                        f"Analysis incomplete after {meta.get('react_iterations', 0)} iterations. "
                        f"Error: {error_log.error_message or error_log.error_type}"
                    ),
                    "confidence": "low",
                    "confidence_reason": "Agent did not complete full investigation",
                    "code_reference": {
                        "file": "unknown",
                        "repo": repo_info.get("repo", error_log.service_name),
                    },
                }
            else:
                rc = report_data["root_cause"]
                if isinstance(rc, dict) and "code_reference" not in rc:
                    rc["code_reference"] = {
                        "file": "unknown",
                        "repo": repo_info.get("repo", error_log.service_name),
                    }
            report_data.setdefault("impact_assessment", {
                "affected_service": error_log.service_name,
                "affected_environment": error_log.environment or "production",
            })
            report_data.setdefault("suggested_solutions", [
                {"priority": 1, "effort": "medium", "title": "Investigate root cause", "description": "Review the stack trace and recent commits for this service."}
            ])

            raise _FinishRCA(RCAReport(**report_data))

        logger.warning("Unknown tool: %s", name)
        return {"error": f"Unknown tool: {name}"}

    def _persist(self, report: RCAReport, error_log_id: str) -> None:
        """Update error_logs and insert into rca_reports."""
        report_json = json.dumps(report.model_dump(mode="json"))
        try:
            execute(
                """UPDATE error_logs
                   SET rca_status = 'completed',
                       rca_completed_at = NOW(),
                       rca_result = %s
                   WHERE id = %s""",
                (report_json, error_log_id),
            )
        except Exception as exc:
            logger.warning("Could not update error_logs: %s", exc)

        try:
            execute(
                """INSERT INTO rca_reports (rca_id, error_log_id, service_name, generated_at, report)
                   VALUES (%s, %s, %s, %s, %s)
                   ON DUPLICATE KEY UPDATE
                       report = VALUES(report),
                       generated_at = VALUES(generated_at)""",
                (
                    report.rca_id,
                    error_log_id,
                    report.service_name,
                    report.generated_at,
                    report_json,
                ),
            )
        except Exception as exc:
            logger.warning("Could not insert into rca_reports: %s", exc)

        try:
            append_rca_history(report.service_name, {
                "rca_id": report.rca_id,
                "generated_at": report.generated_at,
                "root_cause_summary": report.root_cause.summary,
                "confidence": report.root_cause.confidence,
                "cross_service": report.service_boundary_crossed,
                "upstream_service": report.upstream_service,
            })
        except Exception as exc:
            logger.warning("Could not append rca_history: %s", exc)
